...
export default function AdminMatchRowActions({ match }: { match: IMatch }) {
  ...
  const handleStatusChange = async (status: MatchStatus) => {
    ...
    setIsUpdating(true);
    const res = await updateMatchStatus(match.id, status as MatchStatus);
    setIsUpdating(false);
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    const res = await deleteUser(match.id);
    setIsDeleting(false);
    setOpenDelete(false);
