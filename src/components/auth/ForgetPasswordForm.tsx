...
const ForgetPasswordForm = () => {
  ...
  useEffect(() => {
    if (state?.success || state?.message === "OTP sent to your email") {
      ...
      router.push(`/verify-otp?email=${encodeURIComponent(state.data.email)}`)
    ...
    <>
      <form action={formAction} className="space-y-5">
        <div className="space-y-2">
          ...
          {errorMessage && <p className={`text-sm  ${errorMessage === "OTP sent to your email" ? "text-green-300" : "text-destructive"}`}>{errorMessage}</p>}
        </div>
